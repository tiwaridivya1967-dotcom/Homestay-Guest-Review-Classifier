...
export default function AnalyzerWorkspace() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/reviews')
      .then(res => {
        if (!res.ok) throw new Error("Failed to load pipeline datasets.");
        return res.json();
      })
      .then(data => {
        setReviews(data);
  ...
  if (error) return <div className="text-center py-20 font-semibold text-red-500 bg-red-50 max-w-xl mx-auto rounded-2xl border border-red-200">⚠️ Connection Blocked: Ensure your backend server is active on terminal node port 5000!</div>;
  ...
  return (
    <div className="space-y-10 animate-fadeIn text-gray-800">
      ...
      <div>
        <div className="mb-4">
          <h2 className="text-xl font-black text-gray-900 tracking-tight">Monitored Properties</h2>
          <p className="text-xs text-gray-400">Live background feeds from your active property listings</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Card 1 */}
          <div className="relative h-48 rounded-2xl overflow-hidden shadow-sm group border border-gray-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Dynamic Data Analytical Graph Bars */}
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm lg:col-span-2 space-y-6">
          <div>
            <h3 className="text-lg font-bold text-gray-900">Hospitality Aspect Weight Matrix</h3>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full bg-gray-100 h-6 rounded-xl overflow-hidden">
          ...
          {/* High Value Informative Stats Subgrid */}
          <div className="grid grid-cols-3 gap-4 border-t border-gray-100 pt-6 text-center">
            <div>
              <span className="block text-xl font-black text-gray-900">{totalReviews}</span>
        ...
        {/* Action Panel: Feed Data Ingestion Control */}
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          <h3 className="text-lg font-bold text-gray-900">Data Ingestion Engine</h3>
          <p className="text-xs text-gray-400 mb-4">Simulate live reviewer parameters</p>
          
          <form onSubmit={handleCreateReview} className="space-y-4">
            <input type="text" placeholder="Guest Name" value={guestName} onChange={e => setGuestName(e.target.value)} required className="w-full p-3 border border-gray-200 bg-gray-50/50 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
            
            <div className="grid grid-cols-2 gap-2">
              <select value={selectedTheme} onChange={e => setSelectedTheme(e.target.value)} className="p-2.5 border border-gray-200 bg-white text-xs font-semibold rounded-xl text-gray-600">
                <option value="cleanliness">✨ Cleanliness</option>
                <option value="host">🛎️ Host</option>
                <option value="location">📍 Location</option>
              </select>
              <select value={selectedSentiment} onChange={e => setSelectedSentiment(e.target.value)} className="p-2.5 border border-gray-200 bg-white text-xs font-semibold rounded-xl text-gray-600">
                <option value="positive">🟢 Positive</option>
                <option value="negative">🔴 Negative</option>
              </select>
            </div>

            <textarea placeholder="Type operational review narrative here..." value={newComment} onChange={e => setNewComment(e.target.value)} required className="w-full p-3 border border-gray-200 bg-gray-50/50 text-sm rounded-xl h-20 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 bg-gray-50/80 border-b border-gray-100 flex justify-between items-center">
          <h3 className="font-bold text-sm text-gray-900">Active Review Logs Timeline</h3>
          <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full">Systems Linked</span>
        </div>
        <div className="divide-y divide-gray-100">
          {reviews.map(review => (
            <div key={review.id} className="p-6 hover:bg-gray-50/20 transition-colors flex justify-between items-center">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h4 className="font-bold text-sm text-gray-900">{review.guest}</h4>
                  <span className={`text-[9px] uppercase font-black tracking-widest px-2 py-0.5 rounded-full ${review.sentiment === 'positive' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100'}`}>
                    {review.sentiment}
                  </span>
                </div>
                <p className="text-xs text-gray-500 italic">"{review.comment}"</p>
              </div>
              <span className="text-xs font-bold bg-gray-50 text-gray-600 px-3 py-1 rounded-xl border border-gray-200/60 capitalize">
                {review.theme}
              </span>
            </div>
          ))}
